...
class CorrmatrixTaskParams(BaseModel):
    url: HttpUrl
    columns1: str = Field(..., regex=r'(^\d+(-\d+)?(?:,\d+(?:-\d+)?)*$)|(^\*$)', title='Первый набор столбцов', description='Пример: 2-4,6,8')
    columns2: str = Field(..., regex=r'(^\d+(-\d+)?(?:,\d+(?:-\d+)?)*$)|(^\*$)', title='Второй набор столбцов', description='Пример: 2-4,6,8')
